...
namespace Earendil_GPS {
    ...
    void createTask_vGPS_UpdateData(void){
        TaskHandle_t* task_handle_ptr = &Earendil_Handles->GPS_Handles.task_vGPS_UpdateData;
        xTaskCreate(
            vGPS_UpdateData,                                // Task function
            "vGPS_UpdateData",                              // Task name (for debugging)
