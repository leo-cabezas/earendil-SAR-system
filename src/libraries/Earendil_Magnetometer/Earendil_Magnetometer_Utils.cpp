...
namespace Earendil_Magnetometer {
    ...
    double getHeading(double calibrated[3]){
        double heading = (-1) * atan2(calibrated[0], calibrated[1]) * (180.0 / M_PI);
        return heading;
    }
    ...
    void applyCalibration(double calibrated[3]){
        for (int i = 0; i < 3; i++){
            double no_hard = magmeter.filtered[i] - magmeter.hard_iron[i]; //subtract by the hard iron calibration scale
