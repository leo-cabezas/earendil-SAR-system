...
void calibrateMagnetometer()
{
  ...
  while (millis() - start < CALIBRATION_TIME) //while the current timer is less than the calibration timer
  {
    ...
    mag.getEvent(&event); //this grabs the current magnetometer readings
...
float computeHeading(float calibrated[3])
{
  float heading = atan2(calibrated[1], calibrated[0]) * 180.0 / PI; //get the heading in degrees
